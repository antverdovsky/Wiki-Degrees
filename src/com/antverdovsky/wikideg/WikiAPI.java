...
class ExportLinksFetcher implements AbstractLinkFetcher {
	@Override
	/**
	...
	public ArrayList<String> getLinks(String article, String target) 
		...
		String url = WikiFetch.getExportURL(article);
		String export = WikiFetch.getData(url);

		WikiParse.parseLinksExport(export, allLinks, target);

		return allLinks; // Return all of the links when done
	}
}

class JSONLinksFetcher implements AbstractLinkFetcher {
	@Override
	/**
	 * Gets all (or some, if the target is found) of the links on a Wikipedia
	 * article of the specified name.
	...
	public ArrayList<String> getLinks(String article, String target) 
		...
		while (!continueToken.isEmpty()) {
			...
			continueToken = WikiParse.parseLinksJSON(json, allLinks, target);
		}
...
class BacklinksFetcher implements AbstractLinkFetcher {
	...
	public ArrayList<String> getLinks(String article, String target) 
		...
		while (!continueToken.isEmpty()) {
			...
			continueToken = WikiParse.parseBacklinksJSON(json, allBacklinks, 
					target);
		}
