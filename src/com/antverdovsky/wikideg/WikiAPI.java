...
public class WikiAPI {
	 ...
	 * @return A 2D List of all of the backlinks. Each row in the list 
	 *         contains a list of article titles, with at most 500 articles
	 *         per list.
	 * @throws IOException If the article data could not be fetched.
	 */
	public static ArrayList<ArrayList<String>> getAllBacklinks(String article)
			throws IOException {
		...
		// page).
		LinksResult result = new LinksResult(null, "0|0");
		...
		while (!result.getLContinue().isEmpty()) {
			...
			url = WikiFetch.getBacklinksURL(article, result.getLContinue());
			json = WikiFetch.getData(url);

			// Parse the JSON data from the URL and add it to the list
			result = WikiParse.parseBacklinks(json);
